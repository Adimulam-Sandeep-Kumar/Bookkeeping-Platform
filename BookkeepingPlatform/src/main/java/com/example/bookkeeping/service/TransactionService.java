...
public class TransactionService {
    ...
    public void processTransaction(Transaction transaction) {
        BigDecimal finalAmount = transaction.getAmount();
        
        if (!transaction.getCurrency().equals("USD")) {
            BigDecimal convertedAmount = currencyConversionService.convert(transaction.getCurrency(), "USD", transaction.getAmount());
            finalAmount = convertedAmount; // Use converted amount for internal processing
        }

        transaction.setAmount(finalAmount); // Store the converted amount
        transactionRepository.save(transaction); // Save transaction to database
    }
}
